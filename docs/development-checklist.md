...
- [x] 新增 GitHub Actions 镜像构建与部署工作流
- [x] 固化远端 `.env` 镜像切换与 `docker compose pull && up -d` 流程
- [x] 补充服务器环境变量模板说明
- [x] 补充部署文档和仓库入口文档链接

### 维护要求

- 变更镜像名、部署目录或 compose 结构时，同步更新 workflow、远端脚本和部署文档
- 新增生产依赖的 Secrets 时，同步更新 `docs/deployment.md`
